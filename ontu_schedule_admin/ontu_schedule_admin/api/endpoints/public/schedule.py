...
async def get_schedule(
    ...
    group = await read_group(group_id=group_id)
...
async def get_tomorrow_schedule(
    ...
    day_schedule = await get_day_schedule(
        group=await read_group(group_id=group_id),
        for_day=date,
        ignore_cache=ignore_cache,
    )
...
async def get_specific_day_schedule(
    request: HttpRequest,  # noqa: ARG001
    group_id: pydantic.UUID4,
...
async def get_teacher_schedule(
    ...
    teacher = await read_teacher(teacher_id=teacher_id)
...
async def get_teacher_day_schedule(
    request: HttpRequest,  # noqa: ARG001
    teacher_id: pydantic.UUID4,
    for_day: datetime.date,
    ignore_cache: bool = False,
